...
> Please note that the type declarations are in an early stage and not complete. (Pull requests welcome!)

While this addon should make it easy for anyone to get started with Typescript 2.3 in your Ember CLI app, it hasn't been fully tested and won't catch all errors in your code (although it will help a lot!).
...
For better formatted type errors, check your code by running `tsc`:
...
### No need to rewrite your app to use TypeScript
...
[TypeScript 2.1](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-2-1.html) comes with `keyof` and lookup types, this enables us to statically check if an object has certain properties, and also retrieve the type of those properties:
...
### `this parameter` context in computed properties

The proper `this` context is not afvailable inside computed property definitions yet:
...
- services injection via initializers: 
...
https://github.com/emberwatch/ember-cli-typescript/issues/37
(^ module resolution doesnt work, which is the reason for module unification?)
